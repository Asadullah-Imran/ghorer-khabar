...
import { User, Utensils, PauseCircle, XCircle, CalendarClock } from "lucide-react";

export default function ActiveSubscriptionCard({ subscription }: { subscription: any }) {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow group">
      <div className="flex flex-col md:flex-row gap-6">
        ...
        <div className="relative shrink-0">
          ...
          <span className={`absolute top-2 left-2 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider text-white ${
          ...
          <div>
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                    <div className="relative w-5 h-5 rounded-full overflow-hidden border border-gray-200">
                        <Image src={subscription.chefImage} alt="chef" fill className="object-cover" />
                    </div>
                    <p className="text-sm text-gray-500">
                    Chef: <span className="font-semibold text-teal-700">{subscription.chefName}</span>
                    </p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-xl border-l-4 border-yellow-400 mt-4">
              <div className="flex justify-between items-center mb-1">
                <p className="text-xs text-gray-500 font-bold uppercase tracking-wider">Next Delivery</p>
                <p className="text-[10px] font-bold text-teal-700 bg-teal-50 px-2 py-0.5 rounded-md">
                    Renew: {subscription.renewalDate}
                </p>
              </div>
              <p className="text-sm font-semibold text-gray-900">{subscription.nextDelivery}</p>
