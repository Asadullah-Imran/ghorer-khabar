...
import { ChefHat, Home, Menu, Search, ShoppingBag, User, X } from "lucide-react";
    ...
    return (
      <>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 py-3 px-6 flex justify-between z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          <Link
            href="/feed"
            className={`flex flex-col items-center gap-1 ${pathname === "/feed" ? "text-teal-600" : "text-gray-400"}`}
          >
            <Home size={22} />
          ...
          <Link
            href="/explore"
            className={`flex flex-col items-center gap-1 ${pathname === "/explore" ? "text-teal-600" : "text-gray-400"}`}
          >
            <Search size={22} />
