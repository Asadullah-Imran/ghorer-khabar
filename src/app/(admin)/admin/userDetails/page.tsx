...
import { 
  ...
  RotateCcw, Edit3, ShieldOff, CheckCircle2, Eye, Map
} from 'lucide-react';
    ...
    <div className="flex h-screen bg-background-dark text-white font-display overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto flex flex-col">
        ...
        <div className="p-8 space-y-6 flex-1">
          {/* Back Navigation */}
          <Link href="/admin/users" className="flex items-center gap-2 text-sm text-text-muted hover:text-white transition-colors">
            <ArrowLeft size={16} /> User Management / <span className="text-white">User Details</span>
          </Link>
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  } text-[10px] font-bold px-2 py-0.5 rounded uppercase`}>{user.role}</span>
                  <span className={`${
                    user.status === 'Active'
                      ? 'bg-primary/10 text-primary'
                      : 'bg-red-500/10 text-red-400'
                  } text-[10px] font-bold px-2 py-0.5 rounded uppercase`}>{user.status}</span>
                </div>
                <div className="flex items-center gap-4 text-xs text-text-muted">
                  <span className="flex items-center gap-1"><ShieldOff size={12}/> {user.id}</span>
                  <span className="flex items-center gap-1"><MapPin size={12}/> Uttara, Dhaka</span>
                  <span>Joined {user.joinDate}</span>
                </div>
              </div>
            </div>
            <div className="flex gap-3">
              <ActionButton icon={<RotateCcw size={16}/>} label="Reset Password" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <StatCard label="Total Orders" value={user.orders} sub={user.revenue} subColor="text-text-muted" />
            ...
            <div className="bg-surface-dark border border-border-dark p-5 rounded-xl">
              ...
              <div className="flex items-center gap-2 font-bold mb-2">
                {user.status === 'Active' ? 'Active' : 'Inactive'} <CheckCircle2 size={16} className={user.status === 'Active' ? 'text-primary' : 'text-red-400'} />
              </div>
              <div className="w-full bg-background-dark h-1.5 rounded-full overflow-hidden">
                <div className={`h-full w-full ${user.status === 'Active' ? 'bg-primary shadow-[0_0_8px_rgba(17,212,180,0.5)]' : 'bg-red-400 shadow-[0_0_8px_rgba(239,68,68,0.5)]'}`} />
              </div>
            </div>
          </div>
              ...
              <SectionBox title="Registered Address">
                <div className="flex gap-6">
                  <div className="flex-1 space-y-4">
                    <div>
                      <p className="text-[10px] uppercase font-bold text-text-muted mb-1">Full Address</p>
                      <p className="text-sm leading-relaxed">House 45, Road 12, Sector 4<br/>Uttara Model Town, Dhaka-1230<br/>Bangladesh</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase font-bold text-text-muted mb-1">Zone / Region</p>
            ...
            <div className="space-y-6">
              <SectionBox title="Recent Orders">
                <div className="space-y-4">
                  <OrderRow id="#ORD-9921" price="৳ 450" status="PREPARING" time="Today, 10:30 AM" />
                  <OrderRow id="#ORD-9918" price="৳ 1,200" status="DELIVERED" time="Yesterday" />
                  <OrderRow id="#ORD-9840" price="৳ 320" status="DELIVERED" time="2 days ago" />
                  <button className="w-full text-center text-xs font-bold text-primary mt-4 uppercase tracking-widest hover:text-primary/80 transition-colors">View Full History</button>
                </div>
              </SectionBox>
              ...
              <SectionBox title="Audit Trail">
                <div className="space-y-6 relative before:absolute before:left-2 before:top-2 before:bottom-2 before:w-px before:bg-border-dark">
                  <AuditItem title="Profile updated by Admin (You)" sub="Today, 10:45 AM • Changed phone number" color="border-primary" />
                  <AuditItem title="Suspicious Login Attempt" sub="Yesterday, 02:15 AM • IP 192.168.1.1" color="border-orange-500" />
                  <AuditItem title="KYC Documents Approved" sub="Oct 20, 2023 • By System Automator" color="border-green-500" />
                </div>
              </SectionBox>
            </div>
          </div>
  ...
  return (
    <div className="bg-surface-dark border border-border-dark rounded-xl overflow-hidden">
      <div className="px-6 py-4 border-b border-border-dark flex justify-between items-center">
        ...
        {edit && <button className="text-xs text-primary font-bold hover:text-primary/80 transition-colors">Edit</button>}
        {viewAll && <button className="text-xs text-primary font-bold hover:text-primary/80 transition-colors">View All →</button>}
      </div>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2">
        <span className={`text-sm ${muted ? 'text-text-muted italic' : 'font-medium'}`}>{value}</span>
        {verified && <CheckCircle2 size={14} className="text-primary" />}
        {link && <ExternalLink size={14} className="text-primary cursor-pointer" />}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      <div className="aspect-video bg-background-dark rounded-lg border border-border-dark flex items-center justify-center relative">
        ...
        {checked && <div className="absolute top-2 right-2 w-4 h-4 bg-primary rounded-full flex items-center justify-center border-2 border-background-dark"><CheckCircle2 size={10}/></div>}
      </div>
      <p className="text-[10px] text-center font-bold text-text-muted uppercase">{label}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex justify-between items-start">
      ...
      <div className="text-right">
        ...
        <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded ${status === 'PREPARING' ? 'bg-secondary/10 text-secondary' : 'bg-primary/10 text-primary'}`}>{status}</span>
      </div>
    </div>
  );
}
...
function AuditItem({ title, sub, color }: any) {
  return (
    <div className="pl-6 relative">
      <div className={`absolute left-0 top-1 w-4 h-4 rounded-full bg-surface-dark border-2 ${color} z-10`} />
...
function TabItem({ label, active }: { label: string, active?: boolean }) {
  return (
    <button className={`pb-4 text-xs font-bold uppercase tracking-wider transition-colors ${active ? 'text-primary border-b-2 border-primary' : 'text-text-muted hover:text-white'}`}>
      {label}
    </button>
  );
}